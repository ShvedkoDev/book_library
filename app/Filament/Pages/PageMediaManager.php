        ...
        return $table
            ...
            ->actions([
                ...
                DeleteAction::make()
                    ...
                    ->color(fn ($record) => $record->pages_count > 0 ? 'danger' : 'warning')
                    ->before(function ($record, DeleteAction $action) {
                        ...
                        if ($record->pages_count > 0) {
                            ...
                            Notification::make()
                                ->warning()
                                ->title('File in use!')
                                ->body("This file is currently used by {$record->pages_count} page(s). Please review the pages using this file before deleting.")
                                ->persistent()
                                ->send();
                        }
                    })
                    ->action(function ($record) {
                        $pagesCount = $record->pages_count;
                        ...
                        if (Storage::disk('public')->exists($record->path)) {
                            ...
                            if ($pagesCount > 0) {
                                Notification::make()
                                    ->warning()
                                    ->title('File deleted')
                                    ->body("The file '{$record->filename}' has been deleted. Note: {$pagesCount} page(s) were using this file and now have broken references.")
                                    ->persistent()
                                    ->send();
                            } else {
                                Notification::make()
